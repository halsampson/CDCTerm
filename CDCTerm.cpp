...
bool processKey() { // returns true if Esc seq
  ...
  while (1) {
    try {
      while (1) {
        ...
        while (_kbhit()) {
          if (processKey()) break;
          while (++pasteCount >= 80 && _kbhit()) { // pasting, not typing; process a line - Must have CR at end of pasted text!!!
            if (pasteCount == 80) setInputEcho(false);
            char buf[64 * 2 + 1]; // two USB buffers
            fgets(buf, sizeof(buf)-1, stdin); // to buffer or newline;  echoed locally
